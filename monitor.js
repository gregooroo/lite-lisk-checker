...
module.exports = class Monitor {
  ...
  async checkNode(serverId) {
    if (this.liskClient) { // Lisk testnet&mainnet
      ...
      const i = this.nodes.map(n => n.id).indexOf(serverId); // Index of node (serverId) in array
      if (i !== -1) {
  ...
  async checkNodes() {
    // Read all nodes, check if they are available to connect,
    // check their height vs liskAPI node (HQ nodes or custom config).
